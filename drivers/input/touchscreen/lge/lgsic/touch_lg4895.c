...
int lg4895_tc_driving(struct device *dev, int mode)
{
	...
	do {
		...
		touch_msleep(20);
		if( lge_get_boot_mode() == LGE_BOOT_MODE_QEM_910K || lge_get_boot_mode() == LGE_BOOT_MODE_PIF_910K) {
			break;
		}
		...
		if ( ((0x10==running_status || 0xF==running_status || 0x0==running_status) && (LCD_MODE_STOP!=mode)) || /* u0,u2,u3 */
			((mode==LCD_MODE_STOP) && (0x0!=running_status))) {
				TOUCH_E("Miss command [mode:%d][status:%X][retry:%d]\n",
					mode, running_status, ++retry);
	...
	if ( retry == 3 )
		TOUCH_E("%s : command fail", __func__);
